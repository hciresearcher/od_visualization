...
class Showmap:
    ...
    def heatmap(self):
        ...
        # OD 핕터링
        if move_text == "유입+유출":
            filter_file_1 = glob('D://Data_Visualization//Data//kt_전처리//*.pkl')
        else:
        ...
        if move_text == '유출':
            ...
            rip_data = pd.concat([rip_data, add_data], axis=0)
        ...
        rip_data.drop_duplicates(['gender', 'age', 'od', 'ttime', 'dist', 'personcount','nationality', 
                                  'oriYear', 'desYear', 'oriMonth', 'desMonth', 'oriHour','desHour'],
        ...
        end1 = time()
        print(f"test 7 걸린 시간은 {end1-start1:.4f}초입니다.")
        ...
        ###### test8 ######        
        start1 = time()                
        groupby_data = allData.groupby('polycode')['ttime'].mean().to_frame('평균 이동시간(분)').reset_index()
        dist_data = allData.groupby('polycode')['dist'].mean().to_frame('평균 이동거리(m)').reset_index()
        person_data = allData.groupby('polycode')['personcount'].sum().to_frame('이동인구수(명)').reset_index()
        end1 = time()
        ...
        groupby_data['평균 이동시간(분)'] = round(groupby_data['평균 이동시간(분)'], 2)
        groupby_data['평균 이동거리(m)'] = round(groupby_data['평균 이동거리(m)'], 2)
        groupby_data['이동인구수(명)'] = round(groupby_data['이동인구수(명)'], 0)

        merge_gpd = self.polydata.merge(groupby_data, how="left", on="polycode")
        
        max_color = int(groupby_data[scatter_text].max())
        avg_color = int(groupby_data[scatter_text].mean())
        
        del groupby_data, dist_data, person_data
        gc.collect()
        
        end1 = time()
        print(f"test 9 걸린 시간은 {end1-start1:.4f}초입니다.")

        ###### test10 ######
        start1 = time()        
        merge_gpd.dropna().reset_index(drop=True, inplace=True)
        end1 = time()
        print(f"test 10 걸린 시간은 {end1-start1:.4f}초입니다.")
    ...
    def flowmap(self):
        ...
        all_data.drop(['oriYear', 'desYear', 'oriMonth', 'desMonth', 'oriHour', 'desHour'], axis=1, inplace=True)
